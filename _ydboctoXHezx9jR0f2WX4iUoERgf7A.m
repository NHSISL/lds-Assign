...
;; ---------------------------------------------------------
;; Generated M code maintains cross reference for oid column in pg_catalog.pg_type table
;; ---------------------------------------------------------
...
xrefPlan(cursorId)
    ...
    NEW aimglobal SET aimglobal=$$XREFDATA^%YDBAIM(base,.subs,"|",30,0,0,1,2,0,0)
    ; Now that xref building is complete, cleanup saved cancel related state information
    KILL %ydboctoCancel("pg_catalog.pg_type","oid","aimXref")
    ; Put the comment in again, in case it's deleted after the metadata is created but before the xref is built
    NEW comment SET comment="xref for table pg_catalog.pg_type column oid"
    SET @aimglobal@("comment-30")=comment
    ; And mark that we are done with building xref so we won't try to build again
    SET ^%ydbAIMOctoCache("pg_catalog.pg_type","oid","completed?")=1
    ...
    SET ^%ydbAIMOctoCache("pg_catalog.pg_type","oid","location")=$name(@aimglobal@(30))
    QUIT
    ;
...
xrefMetadata ; called by src/physical/emit_physical_plan.c after xref plan generation
    ...
    NEW aimglobal SET aimglobal=$$XREFDATA^%YDBAIM(base,.subs,"|",30,1,0,1,2,0,0)
    SET ^%ydbAIMOctoCache("pg_catalog.pg_type","oid","location")=$name(@aimglobal@(30))
    SET ^%ydbAIMOctoCache("pg_catalog.pg_type","oid","completed?")=0
    ; Cancelation information for _ydboctoCleanup.m
    SET %ydboctoCancel("pg_catalog.pg_type","oid","aimXref")=""
    NEW comment SET comment="xref for table pg_catalog.pg_type column oid"
    SET @aimglobal@("comment-30")=comment
    QUIT
    ;
...
baseSubs:(base,subs) ; Private entry point for use by other labels in this routine
    NEW global SET global=$NAME(^%ydboctoocto("tables","pg_catalog","pg_type",":"))
